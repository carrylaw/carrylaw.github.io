...
字典类型是Python中一种非序列的存储结构，存储的是**键值对（key:value）**，其存储形式为 d={key1:value1,key2:value2}，以用户名和密码为例admin={'admin01':1234,'admin02':2345,'admin03':3456}，其中<键>是唯一不重复的，<值>可以不唯一。

**<span style="color:#008B8B;">问题2：Python如何操作字典类型数据？</span>**
